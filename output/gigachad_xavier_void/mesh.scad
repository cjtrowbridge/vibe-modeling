...
// 2) Leave scene_mode = 0 while positioning.
...
// 5) Set scene_mode = 1, then render (F6) and export STL.

// -------------------------
// Scene toggles
// -------------------------
// scene_mode:
//   0 = align (show head + cutter)
//   1 = boolean result (head - cutter)
//   2 = head only
//   3 = cutter only
scene_mode = 1;
show_plate = false;
show_axes = true;
show_guides_in_boolean = false; // keep false for final STL export
...
// -------------------------
// Use the voxel-remeshed head for OpenSCAD booleans. The original mesh is not watertight.
// The 2.0mm voxel version was the best balance that passed CGAL without running out of memory.
...
// Scene output
// -------------------------
is_boolean_mode = (scene_mode == 1);

if ((!is_boolean_mode) || show_guides_in_boolean) {
  build_plate();
  axes();
}
...
if (scene_mode == 1) {
  ...
  color([0.75, 0.75, 0.78, 0.90]) head_mesh_positioned();
} else if (scene_mode == 3) {
  color([1.00, 0.35, 0.10, 0.80]) cutter_mesh_positioned();
} else {
