...
- removable wing-roof insert (`part_id = 6`) is a cap-style cover sized to the wing **top-access aperture** (not the full under-tower cavity), so the tower-side cap edge no longer intrudes into the narrow under-tower slot during insertion,
- cap reach is aperture-anchored: X coverage now includes positive side overhang and fan-side Y coverage includes positive overhang, while the tower-side edge remains intentionally retracted by `roof_insert_tower_backoff`,
- three retention lips (both X edges + tower-side Y edge) remain, but lip placement is now tuned closer to aperture walls for better anti-rattle guidance without creating a hard press-fit,
- current insert-fit targets are `roof_insert_clearance = 0.3 mm`, `roof_insert_lip_inset = 0.15 mm`, `roof_insert_lip_t = 1.2 mm`, `roof_insert_cap_overhang_x = 2.0 mm`, `roof_insert_cap_overhang_fan_y = 2.0 mm`, `roof_insert_tower_backoff = 0.3 mm`,
