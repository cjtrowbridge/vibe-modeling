...
## Revision 0004 dimensions, back-wall rail, and top-band ledges (both walls)

- Top-band ledges, both walls (R4 amend, 2026-09-16; height amended to 2 mm on 2026-09-17, user-directed): the solid bands of both side walls directly above their respective upper windows - left `x = 0..3`, right `x = 70..76` - full wall width `y = 0..45`, `z = 43..45` (top-aligned to the rim; the 2026-09-17 amend reduced the z extent from 3 mm to 2 mm) - each extend 3 mm into the cavity, so the left internal face moves `x = 3` -> `x = 6` and the right internal face moves `x = 70` -> `x = 67` (left ledge occupies `x = 3..6`, right ledge `x = 67..70`); the internal cavity channel is 61 mm wide at that band. Both are flat retention ledges with no expected load path. The 3 mm ledge depth meets the 3.0 mm `minimum_internal_edge_width` directly (asserted `top_band_cavity_extension >= minimum_internal_edge_width` in `case_body.scad`); the 2 mm ledge height is a user-directed dimension on a flat retention ledge, asserted at a documented 2.0 mm floor (`top_band_height >= 2.0`); it is now the only documented `minimum_internal_edge_width` exception in this revision (the earlier 1 mm left-shelf exception is retired). Enabled via `top_band_extension_enabled: true` / `top_band_right_enabled: true`, `top_band_cavity_extension: 3.0`, and `top_band_height: 2.0` in `configs/rev_0004.json` (`top_band_height` falls back to 3.0 in configs that do not declare it, so `rev_0001`..`rev_0003` are unaffected).
...
- New material: the rail ledge, 57 x 3 x 3 = +513.000000 mm³ (the tenon volume already existed as back wall), plus both top-band ledges, 2 x (3 x 42 x 2) = +504.000000 mm³ at the 2026-09-17 2 mm height (each ledge's `y = 42..45` portion already exists as back wall): +1,017.000000 mm³ versus revision 0003. No envelope change: the body spans exactly `[0, 0, 0]` to `[76, 45, 45]` as in R3.
- No cut intersects the rail band (`x = 8..65`, `y = 39..45`, `z = 37..40`) or either top-band ledge (left `x = 3..6`, right `x = 67..70`, both `y = 0..45`, `z = 43..45`): the left-wall windows/exit are at `x = 0..3` and the upper (micro-SD) window top is 1 mm below the ledge bottom (`z = 42` vs `z = 43`), the divider top is `z = 18` (24 mm below the ledge bottom and 19 mm below the rail bottom), and the fan air opening, seats, and mount holes are in the right wall only (`x = 70..76`) - the fan bore top is `z = 41.5` (22.5 + 19), the topmost screw seat bottom is `z = 38.5`, so both clear the right ledge bottom at `z = 43` (a 1.5 mm z-clearance on the bore, asserted conceptually by the fan-wall band `z <= 41.5`). The left ledge tip (`x = 6`) is 2 mm clear of the rail start (`x = 8`) and clears the rail band in `z` (ledge `z = 43..45`, rail `z = 37..40`); the right ledge tip (`x = 67`) is 2 mm clear of the rail end (`x = 65`).

### Revision 0004 candidate artifact verification - 2026-09-17 (R4, 3 x 3 mm rail cross-section + 2 mm top-band ledges on both walls, supersedes the R3 record as current candidate; amended in place three times: 2026-09-16 first the 1 mm left shelf, then both 3 mm ledges, and 2026-09-17 the ledge height reduced to 2 mm)

- Source revision/config: mutable `rev_0004` candidate / `configs/rev_0004.json` (now declaring `top_band_extension_enabled: true`, `top_band_cavity_extension: 3.0`, `top_band_right_enabled: true`, and `top_band_height: 2.0`); `rev_0001.json`, `rev_0002.json`, and `rev_0003.json` remain unchanged (older configs keep working through the `is_undef` fallbacks in `defaults.scad`, so `rail_enabled`, `top_band_extension_enabled`, and `top_band_right_enabled` are false for them and their geometry is byte-identical to before).
- Build scope and destination: one printable part built with `scripts/scad_build.py` into `output/micro_cyberdeck_case/`.
- Expected/actual artifacts: 1/1 STL and 17/17 PNG files; 18 total files, no directories or staged `.scad` files.
- Build log: zero OpenSCAD warnings or errors; all dimension, margin, lip, fan-wall, blind-seat, divider, rail, and top-band assertions passed (OpenSCAD: 938 vertices, 471 facets at export; `Volumes: 2` reported as usual for this design).
- STL bounds: `[0, 0, 0]` to `[76, 45, 45]` mm, a 76 x 45 x 45 mm span.
- Signed STL volume: 33,440.77 mm³ across 1,904 triangles; exact +1,017.000000 mm³ versus revision 0003 (32,423.773222 mm³ recorded in the R3 record above) - the 57 x 3 x 3 rail ledge (tenon already interior to the back wall) plus the two 3 x 42 x 2 top-band ledges (each's `y = 42..45` portion already interior to the back wall) - and exactly -252.000000 mm³ (= 2 x 3 x 42 x 1, the removed `z = 42..43` strip, one per ledge) versus the pre-amend `801e0e9` mesh (1,908 triangles, 33,692.773222 mm³); the ledges share only the coplanar face at `y = 42` with the back wall (no solid overlap), so the height reduction removes exactly the `z = 42..43` strips. The exporter reports `Volumes: 2`; this is a known, user-accepted export characteristic of this design and is not chased.
- Installed render review: passed for the 76 x 45 x 45 mm envelope, both top-band ledges (2 mm steps at `z = 43..45`, one along each side wall, visible along the full width inside the cavity), the back-wall rail (57 mm ledge band `z = 37..40` on the back-wall internal face, flush rear tenon), both left-wall windows, the divider band, and the right-wall fan interface.
- Numeric STL verification (temporary probe script under `output/micro_cyberdeck_case/`, deleted after review): passed. On the amended mesh, STL bounds `[0, 0, 0]` to `[76, 45, 45]`, signed volume 33,440.773222 mm³ across 1,904 triangles - exact -252.000000 mm³ (= 2 x 3 x 42 x 1, the removed `z = 42..43` strip, one per ledge) versus the pre-amend `801e0e9` mesh (1,908 triangles, 33,692.773222 mm³), and +378.000000 mm³ versus the mesh extracted from `81dd652` (1,900 triangles, 33,062.773222 mm³ - that commit's rev_0004 state already contained the rail (513.000000 mm³) and the 1 mm left ledge (1 x 42 x 3 = 126.000000 mm³ of cavity material), so the remaining +378.000000 mm³ is the left ledge grown from 1 x 42 x 3 to 3 x 42 x 2 (+252.000000 - 126.000000 = +126.000000) plus the new 2 mm right ledge (+252.000000); the full +1,017.000000 mm³ net over revision 0003 is 513 + 504).
- Governed temporary section probes (temporary `.scad` under `output/micro_cyberdeck_case/` instantiating the production body module with exactly the config's defines, rendered, viewed, and removed with their PNGs): (a) horizontal section at `z = 44` - top-down ortho - shows both side walls' top bands as one continuous 5 mm strip (3 mm wall + 2 mm ledge) spanning the full wall width into the back-wall corner with no seam gap, and the right (fan) wall symmetric with the left; (b) constant-`y` section at `y = 22.5` - front ortho - shows the U-profile with both 2 mm ledge steps at `z = 43..45` (internal faces `x = 6` and `x = 67`), both left-wall windows below them (`z = 3..15` battery exit, `z = 18..42` upper window) untouched, divider band `z = 15..18` unchanged, and the 6 mm fan wall with the ledge step on top, the fan bore top (`z = 41.5`) 1.5 mm below the ledge bottom; the back-rail tenon protrudes from the back-wall face as expected.
- Full-mesh identity check (temporary script, deleted after review): comparing the facet multisets of the amended rebuild (1,904 triangles) and the extracted `81dd652` mesh (1,900 triangles - the first-amend state: 3 x 3 rail plus the 1 mm left ledge, `x = 3..4`, `z = 42..45`) gives 81 added and 77 removed triangles (1,823 shared; net +4). Every added/removed triangle is coplanar with one of the design's structural faces; the exact per-face census (count, that face's `z` span among the diff triangles) is - added: `x = 3` (5, `z = 3..43`), `x = 6` (2, `z = 43..45`), `x = 67` (2, `z = 43..45`), `x = 70` (46, `z = 3..43`), `y = 0` (6, `z = 3..45`), `y = 42` (10, `z = 18..45`), `z = 43` (4), `z = 45` (6); removed: `x = 3` (3, `z = 3..42`), `x = 4` (2, `z = 42..45`), `x = 70` (46, `z = 3..45`), `y = 0` (4, `z = 3..45`), `y = 42` (8, `z = 18..45`), `z = 42` (8), `z = 45` (6). The `x = 6` / `x = 67` / `z = 43` / `x = 4` entries are the new 2 mm ledges' exposed faces (`x = 3..6` / `x = 67..70`, `y = 0..45`, `z = 43..45`) and the old 1 mm left ledge's outer face; the remaining entries are the coplanar re-triangulation OpenSCAD performs when the ledge cubes merge into the faces they touch (CSG regenerates the fan of a merged coplanar face, so the diff reaches down each face below the ledge bottom as well): the `x = 3` / `x = 70` wall-internal faces (the old fans span `z = 3..42` / `z = 3..45` - top at the lower window / the rim where there was no right ledge in `81dd652`; the new fans both span `z = 3..43`, top at the ledge bottom), the `y = 0` front face (both spanning `z = 3..45`) and the `y = 42` back-wall internal face (both spanning `z = 18..45`, the region below the divider band untouched in both meshes), and the replaced rim-top (`z = 45`) fan. No added/removed vertex lies in the open space between the bore top and the ledge bottom (`x = 40..66`, `y = 1..41`, `z = 41.5..43`): that band is genuinely empty cavity in both meshes.
- Structural joins: passed for all pre-amend joins (the extracted `81dd652` mesh and the amended rebuild share 1,823 facets) plus the rail join (unchanged) and both ledge joins (relatively to `81dd652`, the left ledge's depth grew from 1 mm to 3 mm and the right ledge exists for the first time; relatively to the last both-ledge state `801e0e9`, both ledges' height shrank from 3 mm to 2 mm - verified below). The rail occupies a positive 57 x 3 x 3 mm volume across the full 3 mm back-wall thickness - a `minimum_structural_overlap`-meeting continuous engagement along the entire 57 mm seam, with no cuts in the rail band; asserted (`rail_tenon_rear_y() - back_wall_y() == wall_thickness`, `wall_thickness >= minimum_structural_overlap`). Each 2 mm ledge unites to its side wall over a coplanar-union face (the wall's internal face at `x = 3` for the left ledge, `x = 70` for the right ledge, `y = 0..45`, `z = 43..45`) and shares only the coplanar face at `y = 42` with the back wall - per the structural-joins policy that coplanar contact alone is not a structural join, each ledge's single verified support across the full seam is its side-wall union, with no cuts in the band; each is a flat retention ledge with no expected load path, its bottom face a 3 mm (45-degree) cantilever overhang, printable without supports.
- Minimum internal edge/material width: passed with one documented exception (the 2 mm ledge height) - the rail's 3 x 3 mm section and each ledge's 3 mm depth meet the declared 3.0 mm `minimum_internal_edge_width` directly (asserted `rail_section >= minimum_internal_edge_width` and `top_band_cavity_extension >= minimum_internal_edge_width`), each ledge's 2 mm height is a user-directed flat-retention dimension asserted at its documented 2.0 mm floor (`top_band_height >= 2.0`), and every other named member is at or above 3 mm: each internal side-wall face is 5 mm away from the rail ends, the rail-to-rim material is 5 mm, the rail ledge to divider top is 19 mm, the ledge bottom to divider top is 24 mm, and the material behind the rail tenon is 3 mm (the rear wall face itself). The earlier documented 1 mm left-shelf exception is retired; the 2 mm ledge height is the only documented exception in this revision.
...
SHA-256 provenance for the verified revision 0004 candidate (R4 + 3 x 3 rail + 2 mm top-band ledges on both walls; 2026-09-17 amend, supersedes the 2026-09-16 provenance block above):

- Config `rev_0004.json`: `7F7C30CDCA3754EEA5A050769D4BC87EB0B5CBB9381F9E010F5C8F025CED3F9D`
- Entrypoint `main.scad`: `20B10F91D42D4D880944F6917BAEE1D11A116B410D2F93943937227FFEEE8118` (unchanged since R2)
- Defaults `defaults.scad`: `E8890E5A8F3913B07ADCA3C72ABC600023DC19CB8C060F295DB7B791C2F27A24`
- Part source `case_body.scad`: `9F50C6F4CAFE8BDBB4D947CB6499032E37F0D038827074F2050D25FB22FFE744`
- Installed STL: `B1EFAB473EABC6F9F7F317D73FA32D6AED7FEF972D415B4A36A9DFE1CAC9F964`
