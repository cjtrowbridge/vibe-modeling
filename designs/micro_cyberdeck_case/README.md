...
- Top-band ledges, both walls (R4 amend, 2026-09-16, user-directed): the solid bands of both side walls directly above their respective upper windows - left `x = 0..3`, right `x = 70..76` - full wall width `y = 0..45`, `z = 42..45` (upper-window top to the rim) - each extend 3 mm into the cavity, so the left internal face moves `x = 3` -> `x = 6` and the right internal face moves `x = 73` -> `x = 70` (left ledge occupies `x = 3..6`, right ledge `x = 69..72`); the internal cavity channel is 66 mm wide at that band. Both are flat retention ledges with no expected load path. The 3 mm ledge thickness meets the 3.0 mm `minimum_internal_edge_width` directly (asserted `top_band_cavity_extension >= minimum_internal_edge_width` in `case_body.scad`); the earlier documented 1 mm exception is retired. Enabled via `top_band_extension_enabled: true` / `top_band_right_enabled: true` and `top_band_cavity_extension: 3.0` in `configs/rev_0004.json`.
...
- New material: the rail ledge, 57 x 3 x 3 = +513.000000 mm³ (the tenon volume already existed as back wall), plus both top-band ledges, 2 x (3 x 42 x 3) = +756.000000 mm³ (each ledge's `y = 42..45` portion already exists as back wall): +1,269.000000 mm³ versus revision 0003. No envelope change: the body spans exactly `[0, 0, 0]` to `[76, 45, 45]` as in R3.
- No cut intersects the rail band (`x = 8..65`, `y = 39..45`, `z = 37..40`) or either top-band ledge (left `x = 3..6`, right `x = 69..72`, both `y = 0..45`, `z = 42..45`): the left-wall windows/exit are at `x = 0..3` and the upper (micro-SD) window top is flush with the left ledge bottom at `z = 42`, the divider top is `z = 18` (24 mm below the ledge bottom and 19 mm below the rail bottom), and the fan air opening, seats, and mount holes are in the right wall only (`x = 70..76`) - the fan bore top is `z = 41.5` (22.5 + 19), the topmost screw seat bottom is `z = 38.5`, so both clear the right ledge bottom at `z = 42` (a 0.5 mm z-clearance on the bore, asserted conceptually by the fan-wall band `z <= 41.5`). The left ledge tip (`x = 6`) is 2 mm clear of the rail start (`x = 8`) and clears the rail band in `z` (ledge `z = 42..45`, rail `z = 37..40`); the right ledge tip (`x = 69`) is 4 mm clear of the rail end (`x = 65`).

### Revision 0004 candidate artifact verification - 2026-09-16 (R4, 3 x 3 mm rail cross-section + 3 mm top-band ledges on both walls, supersedes the R3 record as current candidate; amended in place the same day, twice: first the 1 mm left shelf, then both 3 mm ledges)

- Source revision/config: mutable `rev_0004` candidate / `configs/rev_0004.json` (now declaring `top_band_extension_enabled: true`, `top_band_cavity_extension: 3.0`, and `top_band_right_enabled: true`); `rev_0001.json`, `rev_0002.json`, and `rev_0003.json` remain unchanged (older configs keep working through the `is_undef` fallbacks in `defaults.scad`, so `rail_enabled`, `top_band_extension_enabled`, and `top_band_right_enabled` are false for them and their geometry is byte-identical to before).
- Build scope and destination: one printable part built with `scripts/scad_build.py` into `output/micro_cyberdeck_case/`.
- Expected/actual artifacts: 1/1 STL and 17/17 PNG files; 18 total files, no directories or staged `.scad` files.
- Build log: zero OpenSCAD warnings or errors; all dimension, margin, lip, fan-wall, blind-seat, divider, rail, and top-band assertions passed (OpenSCAD: 940 vertices, 472 facets at export; `Volumes: 2` reported as usual for this design).
- STL bounds: `[0, 0, 0]` to `[76, 45, 45]` mm, a 76 x 45 x 45 mm span.
- Signed STL volume: 33,692.77 mm³ across 1,908 triangles; exact +1,269.000000 mm³ versus revision 0003 (32,423.77 mm³ recorded in the R3 record above) - the 57 x 3 x 3 rail ledge (tenon already interior to the back wall) plus the two 3 x 42 x 3 top-band ledges (each's `y = 42..45` portion already interior to the back wall) - and exactly +630.000000 mm³ (= 2 x 3 x 42 x 3 new ledge material less the previously-verified 1 x 42 x 3 left shelf already counted) versus the prior pushed commit `81dd652`'s mesh (1,900 triangles, 33,062.7732 mm³). The exporter reports `Volumes: 2`; this is a known, user-accepted export characteristic of this design and is not chased.
- Installed render review: passed for the 76 x 45 x 45 mm envelope, both top-band ledges (3 mm steps at `z = 42..45`, one along each side wall, visible along the full width inside the cavity), the back-wall rail (57 mm ledge band `z = 37..40` on the back-wall internal face, flush rear tenon), both left-wall windows, the divider band, and the right-wall fan interface.
- Numeric STL verification (temporary probe script under `output/micro_cyberdeck_case/`, deleted after review): passed. On the amended mesh, STL bounds `[0, 0, 0]` to `[76, 45, 45]`, signed volume 33,692.773222 mm³ across 1,908 triangles - exact +630.000000 mm³ (= 2 x 3 x 42 x 3 new ledge material) versus the STL extracted from `81dd652` (1,900 triangles, 33,062.773222 mm³).
- Governed temporary section probes (temporary `.scad` under `output/micro_cyberdeck_case/` instantiating the production body module with exactly the config's defines, rendered, viewed, and removed with their PNGs): (a) horizontal section at `z = 43.5` - top-down ortho - shows both side walls' top bands as one continuous 6 mm strip (3 mm wall + 3 mm ledge) spanning the full wall width into the back-wall corner with no seam gap, and the right (fan) wall symmetric with the left; (b) constant-`y` section at `y = 22.5` - front ortho - shows the U-profile with both 3 mm ledge steps at `z = 42..45` (internal faces `x = 6` and `x = 70`), both left-wall windows below them (`z = 3..15` battery exit, `z = 18..42` upper window) untouched, divider band `z = 15..18` unchanged, and the 6 mm fan wall with the 3 mm ledge step on top; the back-rail tenon protrudes from the back-wall face as expected.
- Full-mesh identity check (temporary script, deleted after review): comparing the facet multisets of the STL extracted from `81dd652` (1,900 triangles, the prior pushed state with the 1 mm left shelf) and the amended rebuild (1,908 triangles), the delta is 79 added and 71 removed triangles - the two 3 mm ledges replacing the 1 mm left shelf plus coplanar re-triangulation of the faces they touch (rim top, wall internal faces, back-wall face). No facet delta lies outside the two ledge envelopes `x = 3..6` / `x = 69..72`, `y = 0..45`, `z = 42..45` and their coplanar-union faces.
- Structural joins: passed for all pre-amend joins (the extracted `81dd652` mesh and the amended rebuild share 1,829 facets) plus the rail join (unchanged) and both new ledge joins. The rail occupies a positive 57 x 3 x 3 mm volume across the full 3 mm back-wall thickness - a `minimum_structural_overlap`-meeting continuous engagement along the entire 57 mm seam, with no cuts in the rail band; asserted (`rail_tenon_rear_y() - back_wall_y() == wall_thickness`, `wall_thickness >= minimum_structural_overlap`). Each ledge unites to its side wall over a 45 x 3 mm coplanar-union face (the wall's internal face at `x = 3` or `x = 73`, `y = 0..45`, `z = 42..45`) and overlaps the back wall as a solid 3 x 3 mm cross-section at `y = 42..45` - two verified supports meeting the declared `minimum_structural_overlap` across the full seam, with no cuts in the band; each is a flat retention ledge with no expected load path, its bottom face is a 3 mm (1:1) cantilever overhang, printable without supports.
- Minimum internal edge/material width: passed - the rail's 3 x 3 mm section and each ledge's 3 mm shelf thickness equal the declared 3.0 mm `minimum_internal_edge_width` directly (asserted `rail_section >= minimum_internal_edge_width` and `top_band_cavity_extension >= minimum_internal_edge_width`), and every other named member is at or above 3 mm: each internal side-wall face is 5 mm away from the rail ends, the rail-to-rim material is 5 mm, the rail ledge to divider top is 19 mm, the ledge bottom to divider top is 24 mm, and the material behind the rail tenon is 3 mm (the rear wall face itself). The earlier documented 1 mm left-shelf exception is retired; no documented exceptions remain for this revision.
...
SHA-256 provenance for the verified revision 0004 candidate (R4 + 3 x 3 rail + 3 mm top-band ledges on both walls, 2026-09-16):

- Config `rev_0004.json`: `6762BD40DAC20B9AC922FF218F5ABD44534C3EE6D88E0A05D690F4D7752F8B5C`
- Entrypoint `main.scad`: `20B10F91D42D4D880944F6917BAEE1D11A116B410D2F93943937227FFEEE8118` (unchanged since R2)
- Defaults `defaults.scad`: `DBCB0988E2B95819216A548181C7AE05B20284C81F59EE699F72EE7A454B8A73`
- Part source `case_body.scad`: `056D7CDE590E020E8871D6E82AE2A42F0E75DA3B40BC89F4ECDE425CC7EAAECE`
- Installed STL: `D07D95F23C4472870F9A52A992EB6F7F319DDF44B28F8094FB347B67BADE8FC7`
